...
빌드가 완료되면 `build/distributions/rider-pr-filter-<버전>.zip` 파일이 생성됩니다.

### 버전 올리기

버전은 `gradle.properties`의 `pluginVersion` **한 곳**에서만 관리합니다.

```properties
pluginVersion=1.0.2
```

이 값을 `build.gradle.kts`(Gradle `version`)와 `version.sh`를 통해
`build.sh` / `run.sh` / `package.sh` / `release.sh`가 함께 읽습니다.
`install.sh`는 배포 ZIP에 단독으로 들어가므로, 같이 들어있는 플러그인 ZIP 파일명에서 버전을 읽습니다.
...
4. `build/distributions/rider-pr-filter-<버전>.zip` 파일 선택
5. Rider 재시작
